...
import { useAssignment, useSubmitAssignment } from "../services/assignments";
import { useUser } from "../contexts/UserContext";
import { AssignmentStatus, AssignmentType } from "../types/assignmentResponse ";
import { getAssignmentStatus, getAssignmentType } from "../utils/assignmentUtils";
import ReviewsSection from "./ReviewsSection";
import CodeEditor from "./CodeEditor";
import { getFullName } from "../utils/userUtils";
import Link from "next/link";
import { UserRole } from "@/app/types/review";
...
const Assignment = ({ assignmentId, backUrl }: AssignmentProps) => {
  ...
  const { user } = useUser();
  const isShowSubmitButton = assignment?.type === AssignmentType.SUBMISSION && user?.role === UserRole.STUDENT;
          ...
          <div className="flex items-center gap-6 text-base">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-medium">
                {assignment?.masterAssignment?.isGroupAssignment ? assignment?.group?.name : getFullName(assignment?.user)}</span>
            </div>
          </div>
        </div>
